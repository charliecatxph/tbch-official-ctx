...
mongoose.connect(process.env.DB).then(d => {
    ...
    app.post("/_tbch-api/forget-password", (req, res) => {
        ...
        if (!name || !password || !secu_key) return res.json({ message : "Important fields missing.", success : false });

        User.findOne({ name : name }, async (err, data) => {
            if (err) return res.json({ message : "Error occured!", success : false });
            if (!data) return res.json({ message : "User doesn't exist!", success : false });
    ...
    app.post("/_tbch-api/get-messages", (req, res) => {
        ...
        if (!name) return res.json({ message : "Important fields missing.", success : false });
    ...
    app.post("/_tbch-api/send", (req, res) => {
        ...
        if (!name || !message) return res.json({ message : "Important fields missing.", success : false });
    ...
    app.post("/_tbch-api/verify", (req, res) => {
        try {
            const verify = jwt.verify(req.header("auth-token"), process.env.JWT_SECRET);
            res.json(verify);
        } catch (e) {
            res.json(false)
        }
    ...
    app.post("/_tbch-api/login", (req, res) => {
        ...
        if (!email || !password) return res.json({ message : "Important fields missing.", success : false });
        ...
        User.findOne({ email: email }, async (err, data) => {
            ...
            if (compare) {
                const jwt_hash = jwt.sign({ _id: data._id, name: data.name }, process.env.JWT_SECRET);
                return res.json({
                    message: "Logged in.",
                    success: true,
                    jwt: jwt_hash
    ...
    app.post("/_tbch-api/register", (req, res) => {
        ...
        if (!name || !email || !password || !secu_key) return res.json({ message : "Important fields missing.", success : false });
        ...
        User.findOne({ name: name }, async (err, data) => {
            ...
            User.findOne({ email: email }, async (errx, datax) => {
                if (errx) return res.json({
                    message: `Error occured.`,
                    success: false
                });
                if (datax) return res.json({
